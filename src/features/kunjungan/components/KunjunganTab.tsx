...
import { type KunjunganRow } from '@/lib/kunjungan-types';
import { KpiCard, PctBadge, EmptyState } from './KpiCard';

export function KunjunganTab({ month, data }: { month: string; data: KunjunganRow[] }) {
  ...
  const chartData = data.map(r => ({
    name: String(r.d), total: r.total, target: r.target, pct: r.pct,
    rj: r.rjTotal, ri: r.riTotal, igd: r.igdTotal, mcu: r.mcuTotal,
  }));
  const unitData = [
